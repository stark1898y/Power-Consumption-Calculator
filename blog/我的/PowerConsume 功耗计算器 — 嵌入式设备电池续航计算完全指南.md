...
graph LR
    ...
    subgraph 三端部署
        ...
        D2[网页版] --- D2_1(Flask+JS)
        D3[纯前端版] --- D3_1(HTML+Chart.js)
    end
...
PowerConsume 提供了三种实现版本，覆盖不同的使用场景：

|        版本        |        技术栈        |        文件        |                运行方式                |        适用场景        |
| :----------------: | :------------------: | :-----------------: | :-------------------------------------: | :--------------------: |
|  **桌面版**  |   Python + Tkinter   |     `main.py`     |      `python main.py` 或打包 EXE      | 本地离线使用，功能最全 |
|  **网页版**  | Python + Flask + JS |   `web_app.py`   | `python web_app.py` → localhost:5000 |  团队共享，局域网部署  |
| **纯前端版** | HTML + JS + Chart.js | `docs/index.html` |    直接打开 HTML 或部署 GitHub Pages    |    零依赖，在线体验    |

```mermaid
graph TB
    subgraph "PowerConsume 三端架构"
        Core["核心计算逻辑<br/>电压 / 容量 / 能量 / 续航"]
    end

    subgraph "三端实现"
        Desktop["桌面版<br/>Python + Tkinter<br/>Matplotlib 图表"]
        Web["网页版<br/>Flask 后端 + JS 前端<br/>Matplotlib 服务端渲染"]
        Frontend["纯前端版<br/>HTML + Chart.js<br/>零后端依赖"]
    end

    subgraph "部署方式"
        EXE["打包 EXE<br/>PyInstaller"]
        Server["本地服务器<br/>localhost:5000"]
...
网页版使用 Flask 作为后端，计算逻辑在服务端执行，图表由 Matplotlib 生成后以 Base64 图片返回前端。

### 3. 纯前端版（推荐在线体验）

直接打开 `docs/index.html`，或部署到 GitHub Pages。所有计算逻辑在浏览器端完成，**无需任何后端服务**，零依赖。
...
│   └── script.js              # 网页版前端逻辑
├── docs/
│   └── index.html             # GitHub Pages 纯前端版 (Chart.js)
...
graph LR
    ...
    D --> E{计算模式?}
    E -->|"续航时间"| F["续航天数<br/>= 可用能量 / 每日能耗"]
    E -->|"所需容量"| G["所需容量<br/>= 能量需求 / (电压 × 系数)"]
